...
export class OAuthConnectionService {
  ...
  async completeAuthorization(platform, input = {}, options = {}) {
    ...
    const state = verifySignedOAuthState({ state: input.state, cookieValue: input.stateCookie, secret: stateSecret, now: this.now() });
    if (state.platform !== normalized) throw new Error("OAuth callback platform does not match the authorization request.");
    ...
    if (normalized === "FACEBOOK") {
      ...
      if (pages.length === 1) {
        ...
        await this.channels.updateChannel(channel.id, { status: "CONNECTED", publishingEnabled: false, externalAccountId: pages[0].id });
        return { channelId: channel.id, platform: normalized, status: "CONNECTED", externalAccountId: pages[0].id, pageSelectionRequired: false };
      }
      await this.vault.set(channel.id, { ...credentials, userAccessToken: credentials.accessToken, accessToken: credentials.accessToken, pageId: null });
      await this.channels.updateChannel(channel.id, { status: "DISCONNECTED", publishingEnabled: false, externalAccountId: null });
      return { channelId: channel.id, platform: normalized, status: "PAGE_SELECTION_REQUIRED", externalAccountId: null, pageSelectionRequired: true, pageCount: pages.length };
    }
  ...
  async getConnectionStatus(channelId) {
    ...
    const channel = await this.channels.getChannel(channelId);
    if (!channel) throw new Error("Channel not found.");
    if (this.vault.isConfigured?.() !== true) return { channel, vaultConfigured: false, credentialsConfigured: false, pageSelectionRequired: false };
    const credentials = await this.vault.get(channelId);
    return { channel, vaultConfigured: true, credentialsConfigured: Boolean(credentials?.accessToken), pageSelectionRequired: channel.platform === "FACEBOOK" && Boolean(credentials?.userAccessToken || credentials?.accessToken) && !credentials?.pageId };
  }
  ...
  async listFacebookPages(channelId, options = {}) {
    ...
    const channel = await this.channels.getChannel(channelId);
    if (!channel) throw new Error("Channel not found.");
    if (channel.platform !== "FACEBOOK") throw new Error("Page selection is only available for Facebook channels.");
    const credentials = await this.vault.get(channelId);
    const userAccessToken = credentials?.userAccessToken || (credentials?.pageId ? null : credentials?.accessToken);
    if (!userAccessToken) throw new Error("Reconnect Facebook before selecting a Page.");
    const config = getOAuthConfig("FACEBOOK", options.env || process.env, channel.oauthProfile || "DEFAULT");
  ...
  async selectFacebookPage(channelId, pageId, options = {}) {
    ...
    const updated = await this.channels.updateChannel(channelId, { status: "CONNECTED", publishingEnabled: false, externalAccountId: page.id });
    return { channel: updated, page: { id: page.id, name: page.name } };
  }
  ...
  async #refresh(platform, credentials, config) {
    if (platform === "TIKTOK") return postForm(this.fetchImpl, "https://open.tiktokapis.com/v2/oauth/token/", { client_key: config.clientId, client_secret: config.clientSecret, grant_type: "refresh_token", refresh_token: credentials.refreshToken }, "TikTok");
    if (platform === "YOUTUBE") return postForm(this.fetchImpl, "https://oauth2.googleapis.com/token", { client_id: config.clientId, client_secret: config.clientSecret, grant_type: "refresh_token", refresh_token: credentials.refreshToken }, "Google");
    return credentials;
  }
  ...
  async #fetchFacebookPages(userAccessToken, config) {
    ...
    const body = await fetchJson(this.fetchImpl, url, { headers: { Authorization: `Bearer ${userAccessToken}` } }, "Facebook");
    return (Array.isArray(body?.data) ? body.data : []).filter((page) => typeof page?.id === "string" && page.id && typeof page?.access_token === "string" && page.access_token).map((page) => ({ id: page.id, name: String(page.name || "Facebook Page").slice(0, 160), accessToken: page.access_token }));
  }
}

export function getOAuthConfig(platform, env = process.env, profile = "DEFAULT") {
  const normalized = normalizePlatform(platform);
  const oauthProfile = normalizeOAuthProfile(profile);
  const stateSecret = cleanSecret(env.CLIPFORGE_OAUTH_STATE_KEY, "CLIPFORGE_OAUTH_STATE_KEY");
  if (normalized === "TIKTOK") return { platform: normalized, clientId: cleanSecret(env.TIKTOK_CLIENT_KEY, "TIKTOK_CLIENT_KEY"), clientSecret: cleanSecret(env.TIKTOK_CLIENT_SECRET, "TIKTOK_CLIENT_SECRET"), redirectUri: requireHttpsUrl(env.TIKTOK_REDIRECT_URI, "TIKTOK_REDIRECT_URI"), scopes: normalizeScopes(env.TIKTOK_OAUTH_SCOPES || "video.publish"), stateSecret, oauthProfile };
  if (normalized === "YOUTUBE") {
    const suffix = oauthProfile === "SECONDARY" ? "_SECONDARY" : "";
    const clientIdKey = `GOOGLE_CLIENT_ID${suffix}`;
  ...
  return { platform: normalized, clientId: cleanSecret(env.META_APP_ID, "META_APP_ID"), clientSecret: cleanSecret(env.META_APP_SECRET, "META_APP_SECRET"), redirectUri: requireHttpsUrl(env.META_REDIRECT_URI, "META_REDIRECT_URI"), scopes: normalizeScopes(env.META_OAUTH_SCOPES || "pages_show_list pages_read_engagement pages_manage_posts"), stateSecret, graphVersion, graphBase: "https://graph.facebook.com", oauthProfile };
}
...
export function createSignedOAuthState({ channelId, platform, secret, now = Date.now() }) {
  assertId(channelId, "channel"); const normalized = normalizePlatform(platform); const state = randomBytes(24).toString("base64url");
  const payload = { channelId, platform: normalized, state, expiresAt: now + STATE_TTL_SECONDS * 1000 };
  const encoded = Buffer.from(JSON.stringify(payload), "utf8").toString("base64url"); const signature = sign(encoded, secret);
  return { state, cookieValue: `${encoded}.${signature}` };
}

export function verifySignedOAuthState({ state, cookieValue, secret, now = Date.now() }) {
  const callbackState = cleanRequired(state, "OAuth state", 512); const cookie = cleanRequired(cookieValue, "OAuth state cookie", 4096); const [encoded, signature, ...rest] = cookie.split(".");
  if (!encoded || !signature || rest.length > 0) throw new Error("OAuth state cookie is invalid.");
  const expected = sign(encoded, secret); const actualBytes = Buffer.from(signature); const expectedBytes = Buffer.from(expected);
  if (actualBytes.length !== expectedBytes.length || !timingSafeEqual(actualBytes, expectedBytes)) throw new Error("OAuth state cookie signature is invalid.");
  let payload; try { payload = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8")); } catch { throw new Error("OAuth state cookie payload is invalid."); }
  if (payload?.state !== callbackState) throw new Error("OAuth state mismatch.");
  if (!Number.isFinite(payload?.expiresAt) || payload.expiresAt < now) throw new Error("OAuth authorization request expired.");
  assertId(payload.channelId, "channel"); payload.platform = normalizePlatform(payload.platform); return payload;
}

function normalizeCredentials(platform, token, now, previous = {}) {
  const accessToken = cleanRequired(token?.access_token || token?.accessToken, `${platform} access token`, 10000);
...
function normalizeScopes(value) { const scopes = String(value || "").split(/[\s,]+/).map((scope) => scope.trim()).filter(Boolean); if (scopes.length === 0) throw new Error("At least one OAuth scope is required."); return [...new Set(scopes)]; }
function normalizePlatform(value) { const platform = String(value || "").trim().toUpperCase(); if (!PLATFORMS.has(platform)) throw new Error("Unsupported OAuth platform."); return platform; }
function normalizeOAuthProfile(value) { const profile = String(value || "DEFAULT").trim().toUpperCase(); if (profile !== "DEFAULT" && profile !== "SECONDARY") throw new Error("Unsupported OAuth profile."); return profile; }
function cleanRequired(value, label, maxLength) { const text = String(value || "").trim(); if (!text) throw new Error(`${label} is required.`); return text.slice(0, maxLength); }
function cleanOptional(value, maxLength) { if (value === null || value === undefined || value === "") return null; const text = String(value).trim(); return text ? text.slice(0, maxLength) : null; }
function positiveNumber(value) { const number = Number(value); return Number.isFinite(number) && number > 0 ? number : null; }
function assertId(value, label) { if (!isProjectId(value)) throw new Error(`Invalid ${label} id.`); }
