...
export class TikTokProvider {
  ...
  requirements() {
    return {
      ...
      notes: [
        ...
        "TikTok requires editable post text and express user consent before content is sent.",
      ],
    };
  }
  ...
  async publish(context) {
    ...
    if (!isHttpsUrl(videoUrl)) {
      throw new PublishingProviderError(
        "TikTok server-side publishing requires an HTTPS media URL under a verified TikTok URL property.",
    ...
    if (typeof publishId !== "string" || !publishId) {
      throw new PublishingProviderError(
        ...
        { code: "TIKTOK_INVALID_RESPONSE", retryable: false },
      );
    }
