...
export function getOwnerAuthConfig(env = process.env) {
  ...
  const configured = accessKey.length >= 10 && sessionKey.length >= 32;
...
export function shouldTrustPlatformAuth(env = process.env) {
  const enabled =
    String(env.CLIPFORGE_TRUST_PLATFORM_AUTH || "")
      .trim()
      .toLowerCase() === "true";
  const hostedOnVercel = Boolean(String(env.VERCEL_ENV || "").trim());
  return enabled && hostedOnVercel;
}
...
export function shouldRequireOwnerAuth(env = process.env) {
  ...
  if (shouldTrustPlatformAuth(env)) {
    return false;
  }
...
function isSafeHost(value) {
  return /^[a-z0-9.-]+(?::\d{1,5})?$/i.test(String(value || ""));
}
